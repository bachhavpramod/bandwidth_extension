%% Check if the estimated HB LP coefficients form a 
% unstable filter. If yes, move the unstable poles inside unit circle

poles_mag = abs(pzmap(tf(1,a_hb')));
poles_mag = round(poles_mag,3);

if ~isempty(find(poles_mag>=1))
    a_hb = Get_stable_filter(a_hb');  % zplane to get pole zole(b,a) maps, use row vwctore for b,a
end
if ~isstable(1,a_hb)
